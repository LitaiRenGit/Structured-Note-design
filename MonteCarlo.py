...
@nb.jit(nopython=True,nogil=True)
def simulate_path_eurostyle(S,sigma,random_return):
    S_vec=S*np.ones((random_return.shape[0],))
    for col_idx in range(random_return.shape[1]):
        S_vec*=np.exp(-(sigma**2)/2+random_return[:,col_idx])
    return S_vec.reshape((S_vec.shape[0],1))

@nb.jit(nopython=True,nogil=True)
def simulate_path_amestyle(S,sigma,random_return):
    ...
    for col_idx in range(1,random_return.shape[1]):
        random_return[:,col_idx]=random_return[:,col_idx-1]*np.exp(
            -sigma**2/2+random_return[:,col_idx])
    return random_return

def monte_carlo_pricing_kde(S,sigma,payoff_fcn,sampler,path_num,step_num,kwargs,**kwgs):
    random_return=sampler.resample(size=path_num*step_num).reshape((path_num,step_num))
    stoh_path=simulate_path_eurostyle(S,sigma,random_return)
    return payoff_fcn(stoh_path=stoh_path,**kwargs)

def monte_carlo_pricing_dist(S,sigma,payoff_fcn,sampler,path_num,step_num,kwargs,**kwgs):
    random_return=sampler.rvs(size=path_num*step_num).reshape((path_num,step_num))
    stoh_path=simulate_path_eurostyle(S,sigma,random_return)
    return payoff_fcn(stoh_path=stoh_path,**kwargs)

def monte_carlo_pricing_siml_kde(S,sigma,sampler,path_num,step_num,**kwgs):
    random_return=sampler.resample(size=path_num*step_num).reshape((path_num,step_num))
    return simulate_path_eurostyle(S,sigma,random_return)

def monte_carlo_pricing_siml_dist(S,sigma,sampler,path_num,step_num,**kwgs):
    random_return=sampler.rvs(size=path_num*step_num).reshape((path_num,step_num))
    return simulate_path_eurostyle(S,sigma,random_return)
...
if __name__=='__main__':
    ...
    norm_sampler=norm(loc=r,scale=sigma)
    """
